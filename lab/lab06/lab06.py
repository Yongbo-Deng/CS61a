class Transaction:
    ...
    def report(self):
        ...
        msg = 'no change'
        if self.changed():
            change_num=str(self.before)+'->'+str(self.after)
            "*** YOUR CODE HERE ***"
            if self.after>self.before:
                msg='increased '
            else:
                msg='decreased '
            return str(self.id) + ': ' + msg + change_num
...
class Account:
    ...
    def deposit(self, amount):
        ...
        """
        self.transactions+=[Transaction(self.id,self.balance,self.balance+amount)]
        self.id+=1
...
def make_change(amount, coins):
    ...
    if amount==smallest:
        return [smallest]
    res=[smallest]+make_change(amount-smallest,rest)
    if None in res:
        if rest:
