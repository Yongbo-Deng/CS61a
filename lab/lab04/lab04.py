...
def my_reduce(combiner, seq):
    ...
    def rec_seq(x,y):
        if y>=length:
            ...
            x=combiner(x,seq[y])
            return rec_seq(x,y+1)
    return rec_seq(seq[0],1)
...
def merge(lst1, lst2):
    ...
    def compare_merge(i,j):
        ...
        if j==len2:
            ...
            return [lst2[j]]+compare_merge(i,j+1)
        else:
            return [lst1[i]]+compare_merge(i+1,j)
    return compare_merge(0,0)
...
def count_palindromes(L):
    ...
    2
    """
    return len([x for x in range(len(L)) if L[x].lower()==L[x].lower()[::-1]])
