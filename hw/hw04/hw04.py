def filter(condition, lst):
    ...
    """
    "*** YOUR CODE HERE ***"
    rm=[a for a in lst if not condition(a)]
    for b in rm:
        lst.remove(b)
...
def max_path_sum(t):
    ...
    if is_leaf(t):
        return label(t)
    return label(t)+max([max_path_sum(a) for a in branches(t)])
...
def has_path(t, word):
    ...
    if len(word)==1:
        return word==label(t)
    return label(t)==word[0] and True if True in [has_path(a,word[1:]) for a in branches(t)] else False
