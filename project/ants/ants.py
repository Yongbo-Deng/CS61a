...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            elif place.ant.is_container and not self.is_container:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        dect_place = self.place
        i = 0
        for i in range(1,self.lower_bound+1):
            dect_place = dect_place.entrance
            if dect_place.is_hive:
                return None
        while not dect_place.bees:
            dect_place = dect_place.entrance
            i += 1
            if dect_place.is_hive or i > self.upper_bound:
                return None
        return random_bee(dect_place.bees) # REPLACE THIS LINE
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for b in bee:
            b.reduce_health(amount+damage_die)
        # END Problem 5
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while place_behind:
            if place_behind.ant:
                if not place_behind.ant.is_inspired:
                    place_behind.ant.damage *= 2
                    place_behind.ant.is_inspired = True
                if place_behind.ant.is_container and place_behind.ant.ant_contained and not place_behind.ant.ant_contained.is_inspired:
                    place_behind.ant.ant_contained.damage *= 2
                    place_behind.ant.ant_contained.is_inspired = True
    ...
    def construct(self, place, gamestate):
        if gamestate.have_queen:
            return None
        else:
            gamestate.have_queen = 1
            self.add_to(place)
    def remove_from(self, place):
        return None
        # END Problem 12
...
class SlowThrower(ThrowerAnt):
    ...
    def bee_action(self) :
        def myfun(gamestate):
            if self.slow_turn == 0:
                self.action = SlowThrower.normal_action(self)
    ...
    def throw_at(self, target):
        target.slow_turn = 5
        target.action = SlowThrower.bee_action(target)
