...
def boar_brawl(player_score, opponent_score):
    ...
    points=abs(opponent_ten-player_one)*3
    if points:
        return points
    return 1
    # END PROBLEM 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    pre_num_dice=strategy(0,0)
    for s0 in range(goal):
        for s1 in range(goal):
            nex_num_dice=strategy(s0,s1)
            if pre_num_dice!=nex_num_dice:
                return False
            pre_num_dice=nex_num_dice
    return True
    # END PROBLEM 7
...
def make_averaged(original_function, samples_count=1000):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    def count_averaged(*args):
        total=0
        for i in range(samples_count):
            total+=original_function(*args)
        return total/samples_count
    return count_averaged
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    max_average=0
    for i in range(1,11):
        i_average=make_averaged(roll_dice,samples_count)(i,dice)
        if i_average>max_average:
            max_average=i_average
            max_num=i
    return max_num
    # END PROBLEM 9
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    point=sus_update(0,score,opponent_score)-score
    if point>=threshold:
        num_rolls=0
    return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
