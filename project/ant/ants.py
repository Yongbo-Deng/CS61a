...
class Ant(Insect):
    ...
    def double(self):
        ...
        if not self.doubled :
            self.damage *= 2
            self.doubled = True
        if self.is_container and self.ant_contained and not self.ant_contained.doubled:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        i = 0
        while not p.is_hive:
            if p.bees and self.lower_bound <= i <= self.upper_bound :
...
class SlowThrowerAnt(ThrowerAnt):
    name = 'slow'
    food_cost = 6
    damage = 0
    implemented = False
