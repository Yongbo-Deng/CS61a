...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    "*** YOUR CODE HERE ***"
    def myfun(bindings, env):
        if bindings is nil:
            return nil, nil
        else :
            validate_form(bindings.first, 2, 2)
            name = Pair(bindings.first.first, myfun(bindings.rest, env)[0])
            val = Pair(eval_all(bindings.first.rest, env), myfun(bindings.rest, env)[1])
            return name, val
    names, vals = myfun(bindings, env)
